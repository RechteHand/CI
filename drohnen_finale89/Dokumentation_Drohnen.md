...
#### C. Spatial Partitioning (Grid-Optimierung)
Da bei Tausenden von Drohnen ein ständiger Vergleich jeder Drohne mit jeder anderen (Komplexität $O(N^2)$) die Simulation extrem verlangsamen würde, wird ein **SpatialGrid** (Räumliche Partitionierung) eingesetzt. Die 2D-Welt wird in quadratische Zellen unterteilt. Jede Drohne trägt sich in jedem Frame in die entsprechende Zelle ein. Um Kollisionen oder Signale zu prüfen, fragt eine Drohne nur noch die Drohnen in ihrer eigenen und den direkt benachbarten Zellen (3x3 Block) ab. Dies reduziert die Komplexität der Kollisionserkennung drastisch auf annähernd $O(N)$ und ermöglicht eine flüssige Framerate (60 FPS) selbst bei riesigen Schwärmen.
