...
public class SkyCanvas extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // === 3. Feuerwerk (HINTER den Drohnen) ===
        Feuerwerk.updateAll(frame, width, height);

        for (Feuerwerk fw : Feuerwerk.alle) {
            if (!fw.exploded) {
                // Rakete zeichnen (aufsteigender heller Punkt + kurzer Schweif)
                g2.setColor(new Color(255, 255, 200, 220));
                g2.fillOval((int) fw.rocketX - 2, (int) fw.rocketY - 2, 4, 4);
                // Schweif
                g2.setColor(new Color(255, 200, 100, 100));
                g2.drawLine((int) fw.rocketX, (int) fw.rocketY,
                        (int) fw.rocketX, (int) (fw.rocketY - fw.rocketVy * 4));
            } else {
                // Partikel zeichnen
                for (Feuerwerk.Partikel p : fw.partikel) {
                    Color pc = p.getColor();
                    if (pc.getAlpha() < 5) continue;

                    // Glow
                    g2.setColor(new Color(pc.getRed(), pc.getGreen(), pc.getBlue(),
                            Math.max(0, pc.getAlpha() / 4)));
                    g2.fillOval((int) p.x - 3, (int) p.y - 3, 6, 6);

                    // Kern
                    g2.setColor(pc);
                    g2.fillOval((int) p.x - 1, (int) p.y - 1, 2, 2);
                }
            }
        }
